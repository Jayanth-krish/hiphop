...
class ReactiveMachine {
   ...
   reactSat( signals = undefined ) {
      ...
      /*
      this.nets.forEach((n,i,a) => {
         if(net.isGlobalInputSignalNet){
            this.inObj[n.debug_id] = inSig[n.debug_id];
         }
      });
      */
      /*
      this.nets.forEach((n,i,a) => {
         if (n.LogicalNetType() === "REG") {
            n.fanin_list.slice().forEach(fanin => {
               deleteFan(n, fanin);   
            });
            n.value = reg[n.debug_id];
         }
      });
      */
      ...
      return this.action( reactforSAT );   
   }
   ...
   reactRegSat( reg ) {
      ...
      this.nets.forEach((n,i,a) => {
         ...
         if(n.isGlobalInputSignalNet){
            this.value = this.inObj[n.signal.name];
         }
      });
      return this.action( reactforSAT );   
   }
...
/*    }                                                                */
   ...
   if( nonpropagated_nets > 0 ) {
      if( mach.CausalityErrorTrace === "shallow" ){
         ...
         let { loc, size, signals } = causalityDeep.findCausalityErrorDeep( mach );
      } else {
...
function reactforSAT(mach){
   ...
   for( let i in mach.exec_status_list ) {
      ...
      if( state.kill || state.start ) {
   	   ...
   	   state.prev_suspended = true;
   	   if( state.func_susp ) {
            //state.func_susp.call( state.exec );
         }
      } else if( !state.suspended && state.prev_suspended ) {
   	   state.prev_suspended = false;
   	   if( state.func_res ) {
            //state.func_res.call( state.exec );
         }
      }
   ...
   //console.log("reaction for sat done")
}
